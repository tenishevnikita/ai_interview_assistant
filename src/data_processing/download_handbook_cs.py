#!/usr/bin/env python3
"""
Скрипт для скачивания HTML страниц из Яндекс Хендбука по CS (Введение в компьютерные науки).

Использует Selenium для обхода защиты и поддерживает ручное прохождение капчи.

Использование:
    uv run python -m src.data_processing.download_handbook_cs
    
    # Или напрямую
    uv run python src/data_processing/download_handbook_cs.py
"""
...
OUTPUT_DIR = RAW_DATA_DIR / "cs"

# URL хендбука
HANDBOOK_URL = "https://education.yandex.ru/handbook/vvedenie-v-kompiuternie-nauki"
...
def main():
    ...
    print("=" * 60)
    print("🚀 Яндекс Хендбук CS Downloader")
    print("=" * 60)
