#!/usr/bin/env python3
"""
Скрипт для извлечения текста из HTML файлов Яндекс Хендбука по C++.
...
Использование:
    ...
    # Или напрямую
    uv run python src/data_processing/extract_handbook_cpp.py
"""
...
INPUT_DIR = RAW_DATA_DIR / "cpp"
OUTPUT_DIR = PROCESSED_DATA_DIR / "cpp"

# Базовый URL для формирования ссылок
BASE_URL = "https://education.yandex.ru/handbook/cpp"
    ...
    for elem in content_div.find_all(["h2", "h3", "h4", "p", "ul", "ol", "pre", "blockquote", "details"]):
        if elem.name in ["h2", "h3"]:
            ...
            if code and current_section:
                lang = "cpp"  # По умолчанию C++ для этого хендбука
...
def save_for_rag(all_data: list[dict], output_dir: Path) -> None:
    ...
    text_path = output_dir / "handbook_cpp.txt"
    with open(text_path, "w", encoding="utf-8") as f:
        f.write("\n".join(full_text))
    print(f"✓ Полный текст: {text_path}")

    # 2. JSON
    json_path = output_dir / "handbook_cpp.json"
...
def main():
    ...
    print("=" * 60)
    print("🔍 Извлечение текста из HTML (C++ Handbook)")
    print("=" * 60)
    ...
    if not INPUT_DIR.exists():
        ...
        print("   Сначала запустите: uv run python -m src.data_processing.download_handbook_cpp")
        return
