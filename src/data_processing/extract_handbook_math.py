#!/usr/bin/env python3
"""
Скрипт для извлечения текста из HTML файлов Яндекс Хендбука по Математике.
...
Использование:
    ...
    # Или напрямую
    uv run python src/data_processing/extract_handbook_math.py
"""
...
INPUT_DIR = RAW_DATA_DIR / "math"
OUTPUT_DIR = PROCESSED_DATA_DIR / "math"

# Базовый URL для формирования ссылок
BASE_URL = "https://education.yandex.ru/handbook/math"
...
def extract_text_from_html(html_path: Path) -> Optional[dict]:
    ...
    title = re.sub(r"\s*-\s*Математика$", "", title)
...
def save_for_rag(all_data: list[dict], output_dir: Path) -> None:
    ...
    text_path = output_dir / "handbook_math.txt"
    with open(text_path, "w", encoding="utf-8") as f:
        f.write("\n".join(full_text))
    print(f"✓ Полный текст: {text_path}")

    # 2. JSON
    json_path = output_dir / "handbook_math.json"
...
def main():
    ...
    print("=" * 60)
    print("🔍 Извлечение текста из HTML (Math Handbook)")
    print("=" * 60)
    ...
    if not INPUT_DIR.exists():
        ...
        print("   Сначала запустите: uv run python -m src.data_processing.download_handbook_math")
        return
