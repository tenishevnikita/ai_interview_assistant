#!/usr/bin/env python3
"""
Скрипт для извлечения текста из HTML файлов Яндекс Хендбука по Алгоритмам.
...
Использование:
    ...
    # Или напрямую
    uv run python src/data_processing/extract_handbook_algo.py
"""
...
INPUT_DIR = RAW_DATA_DIR / "algorithms"
OUTPUT_DIR = PROCESSED_DATA_DIR / "algorithms"

# Базовый URL для формирования ссылок
BASE_URL = "https://education.yandex.ru/handbook/algorithms"
...
def process_latex_in_element(elem) -> None:
    """
    Заменяет yfm-latex элементы на читаемый LaTeX.
    
    Inline формулы оборачиваются в $...$, display формулы в $$...$$.
    """
    for latex_elem in elem.find_all(class_="yfm-latex"):
        data_content = latex_elem.get("data-content", "")
        if not data_content:
            continue
        
        try:
            # Декодируем URL-encoded LaTeX
            latex_code = unquote(data_content).strip()
            
            # Определяем тип формулы (inline или display)
            options = latex_elem.get("data-options", "")
            is_display = '"displayMode":true' in options
            
            # Оборачиваем в соответствующие разделители
            if is_display:
                latex_text = f"$${latex_code}$$"
            else:
                latex_text = f"${latex_code}$"
            
            # Заменяем элемент на текст
            latex_elem.replace_with(latex_text)
        except Exception:
...
def extract_sections(content_div, soup) -> list[dict]:
    ...
    # Обрабатываем таблицы (до обработки LaTeX, т.к. таблицы заменяются)
    table_placeholders = {}
    ...
    # Обрабатываем LaTeX формулы во всём контенте
    process_latex_in_element(content_div)
    ...
    for elem in content_div.find_all(["h2", "h3", "h4", "p", "ul", "ol", "pre", "blockquote", "details"]):
        if elem.name in ["h2", "h3"]:
            ...
            if code and current_section:
                lang = "python"  # По умолчанию python для алгоритмов
...
def extract_text_from_html(html_path: Path) -> Optional[dict]:
    ...
    # Убираем суффикс хендбука если есть
    title = re.sub(r"\s*-\s*Алгоритмы$", "", title)
    title = re.sub(r"\s*-\s*Яндекс Хендбук$", "", title)
...
def save_for_rag(all_data: list[dict], output_dir: Path) -> None:
    ...
    text_path = output_dir / "handbook_algorithms.txt"
    with open(text_path, "w", encoding="utf-8") as f:
        f.write("\n".join(full_text))
    print(f"✓ Полный текст: {text_path}")

    # 2. JSON
    json_path = output_dir / "handbook_algorithms.json"
...
def main():
    ...
    print("=" * 60)
    print("🔍 Извлечение текста из HTML (Algorithms Handbook)")
    print("=" * 60)
    ...
    if not INPUT_DIR.exists():
        ...
        print("   Сначала запустите: uv run python -m src.data_processing.download_handbook_algo")
        return
